...
function test() {
  ...
  answer(store, true); stat = store.getSnapshot().questionStats["G5-R-P1-0001"]; assert.deepEqual([stat.attempts, stat.correctCount, stat.wrongCount, stat.currentStreak, stat.bestStreak], [4,3,1,1,2]);
  ...
  const reloaded = progress.createProgressStore({storage}); assert.equal(reloaded.load().ok,true); assert.equal(reloaded.getSnapshot().questionStats["G5-R-P1-0001"].attempts,4);
