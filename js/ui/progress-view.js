...
(function (root) {
  ...
  function renderRecent(sessions, labels) {
    ...
    sessions.forEach(function (session) {
      ...
      details.textContent = labels.grade(session.gradeId) + "・" + labels.skill(session.skill) + "・" + labels.part(session.gradeId, session.skill, session.part) + (session.mode === "review" ? "・復習" : session.mode === "weakness" ? "・弱点練習" : "");
